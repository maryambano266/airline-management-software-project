...
class Airport(models.Model):  #name of airports
...
class AirplaneType(models.Model):
    ...
    basic_cost=models.IntegerField(default=3500)
...
class flight(models.Model):
    ...
    def save(self, *args, **kwargs):
        # If frequency is daily, set last_date to None to indicate daily flights
        if self.frequency == 'daily':
            self.last_date = None
        super(flight, self).save(*args, **kwargs)

    def __str__(self):
        return f"Flight from {self.origin_name} to {self.dest_name} on {self.day}s"
